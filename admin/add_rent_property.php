...
if (isset($_SESSION['add_rent_property_success'])) {
  $successMessage = is_string($_SESSION['add_rent_property_success'])
    ? $_SESSION['add_rent_property_success']
    : null;
  unset($_SESSION['add_rent_property_success']);
}
...
/**
 * Ensure the rent_properties_list table exists with the expected structure.
 */
function add_rent_property_ensure_table(PDO $pdo): void
{
  $pdo->exec(
    'CREATE TABLE IF NOT EXISTS rent_properties_list (
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'location_accessibility'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD location_accessibility LONGTEXT NULL AFTER category");
    }
  } catch (Throwable $e) {
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'project_name'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD project_name VARCHAR(255) NULL AFTER property_type");
    }
  } catch (Throwable $e) {
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'meta_title'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD meta_title VARCHAR(255) NULL AFTER property_title");
    }
  } catch (Throwable $e) {
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'meta_keywords'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD meta_keywords TEXT NULL AFTER meta_title");
    }
  } catch (Throwable $e) {
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'meta_description'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD meta_description TEXT NULL AFTER meta_keywords");
    }
  } catch (Throwable $e) {
  ...
  try {
    $columnCheck = $pdo->query("SHOW COLUMNS FROM rent_properties_list LIKE 'amenities'");

    if ($columnCheck instanceof PDOStatement && $columnCheck->rowCount() === 0) {
      $pdo->exec("ALTER TABLE rent_properties_list ADD amenities LONGTEXT NULL AFTER floor_plans");
    }
  } catch (Throwable $e) {
 ...
 */
function add_rent_property_handle_upload(?array $file, array $allowedMimeMap, string $uploadDir, string $prefix): array
{
...
$pdo = db();
add_rent_property_ensure_table($pdo);
...
if ($isEditing) {
  try {
    $stmt = $pdo->prepare('SELECT * FROM rent_properties_list WHERE id = :id');
  ...
  if ($editingProperty === null) {
    $_SESSION['rent_property_error'] = 'The selected rent property entry could not be found.';
    header('Location: rent_properties_list.php');
    exit;
  }
}
...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    [$newHeroBannerPath, $heroBannerError] = add_rent_property_handle_upload($_FILES['hero_banner'] ?? null, $imageMimeMap, $uploadDir, 'hero_banner_');
    if ($heroBannerError) {
    ...
    [$newBrochurePath, $brochureError] = add_rent_property_handle_upload($_FILES['brochure'] ?? null, $pdfMimeMap, $uploadDir, 'brochure_');
    if ($brochureError) {
    ...
    [$newDeveloperLogoPath, $developerLogoError] = add_rent_property_handle_upload($_FILES['developer_logo'] ?? null, $imageMimeMap, $uploadDir, 'developer_logo_');
    ...
    [$newPermitBarcodePath, $permitBarcodeError] = add_rent_property_handle_upload($_FILES['permit_barcode'] ?? null, $imageMimeMap, $uploadDir, 'permit_barcode_');
    ...
    if ($galleryFiles && isset($galleryFiles['name']) && is_array($galleryFiles['name'])) {
      ...
      for ($i = 0; $i < $countGallery; $i++) {
        ...
        [$path, $error] = add_rent_property_handle_upload($file, $imageMimeMap, $uploadDir, 'gallery_');
        if ($error) {
          $errors[] = $error;
        } elseif ($path) {
          $galleryPaths[]            = $path;
    ...
    for ($i = 0; $i < $maxFloorPlans; $i++) {
      ...
      if ($fileForIndex) {
        [$uploadedFloorPlanPath, $floorPlanError] = add_rent_property_handle_upload($fileForIndex, $imageMimeMap + $pdfMimeMap, $uploadDir, 'floor_plan_');
        if ($floorPlanError) {
    ...
    if (empty($errors)) {
      try {
        if ($isEditing) {
          ...
          $sql = 'UPDATE rent_properties_list SET ' . implode(', ', $setParts) . ' WHERE id = :id';
          ...
          $_SESSION['rent_property_success'] = 'The rent property details have been updated.';
          header('Location: rent_properties_list.php');
          exit;
        }
        ...
        $sql = 'INSERT INTO rent_properties_list (' . implode(', ', $columns) . ') VALUES (' . implode(', ', $placeholders) . ')';
        $stmt = $pdo->prepare($sql);
        foreach ($propertyData as $column => $value) {
          $param = ':' . $column;
          if ($value === null) {
        ...
        $_SESSION['add_rent_property_success'] = 'The rent property has been added successfully.';
        $uploadedFilesToCleanup = [];
        header('Location: add_rent_property.php');
        exit;
      } catch (Throwable $e) {
  ...
  $errors[] = 'An unexpected error occurred while saving the property. Please try again.';
  error_log('Failed to add rent property: ' . $e->getMessage());
}

$pageTitle = $isEditing ? 'Edit Rent Property' : 'Add Rent Property';
$pageDescription = $isEditing
  ? 'Update an existing rent property listing by refining project details, media, and investment highlights.'
  : 'Create a new rent property listing by sharing project details, media, and investment highlights.';

render_head($pageTitle);
echo '<div class="container-fluid layout">';
echo '<div class="row g-0">';
render_sidebar('add-rent-property');
?>
    ...
    <section class="form-section">
      ...
      <div class="col-12">
        <section class="form-section">
          <div class="section-header">
            ...
            <p class="section-subtitle">Summarize the investment rationale to attract renters.</p>
          </div>
          <div class="row g-4">
            <div class="col-md-3">
      ...
      <div class="col-12">
        <div class="d-flex justify-content-end gap-2">
          <?php if ($isEditing): ?>
            <a href="rent_properties_list.php" class="btn btn-outline-secondary">Cancel</a>
          <?php else: ?>
            <button type="reset" class="btn btn-outline-secondary">Reset</button>
          <?php endif; ?>
          <button type="submit" class="btn btn-primary"><?= $isEditing ? 'Update Rent Property' : 'Save Rent Property' ?></button>
        </div>
      </div>
    </form>
  </div>
