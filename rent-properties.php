...
try {
    ...
    $filterOptions['locations'] = array_values(array_filter(array_map(
        ...
        $pdo->query('SELECT DISTINCT property_location FROM rent_properties_list WHERE property_location IS NOT NULL AND TRIM(property_location) <> "" ORDER BY property_location ASC')->fetchAll(\PDO::FETCH_COLUMN)
    )));

    $filterOptions['property_types'] = array_values(array_filter(array_map(
        static fn($value) => is_string($value) ? trim($value) : '',
        $pdo->query('SELECT DISTINCT property_type FROM rent_properties_list WHERE property_type IS NOT NULL AND TRIM(property_type) <> "" ORDER BY property_type ASC')->fetchAll(\PDO::FETCH_COLUMN)
    )));

    $rawBedrooms = $pdo->query('SELECT DISTINCT bedroom FROM rent_properties_list WHERE bedroom IS NOT NULL AND TRIM(bedroom) <> ""')->fetchAll(\PDO::FETCH_COLUMN);
    if ($rawBedrooms) {
    ...
    $filterOptions['completion_years'] = array_values(array_filter(array_map(
        ...
        $pdo->query('SELECT DISTINCT YEAR(completion_date) AS completion_year FROM rent_properties_list WHERE completion_date IS NOT NULL AND completion_date <> "0000-00-00" ORDER BY completion_year ASC')->fetchAll(\PDO::FETCH_COLUMN)
    ...
    if ($propertyCount === 0) {
        ...
        $rentProperties = array_slice($filteredProperties, $offset, $itemsPerPage);
    }
} catch (Throwable $e) {
    $rentProperties = [];
    $propertyCount = 0;
...
$pageEnd = $propertyCount > 0 ? min($offset + count($rentProperties), $propertyCount) : 0;
$propertyLabel = $propertyCount === 1 ? 'rental' : 'rentals';
...
$title = 'Luxury Homes for Rent in Dubai | Verified & Move-In Ready';
...
$meta_tags = '
    <!-- Basic Meta -->
    <meta name="description" content="Rent premium apartments &amp; villas in Dubai’s top areas with HouzzHunt. Verified listings, flexible terms &amp; expert guidance—zero guesswork, just great homes.">
    <meta name="keywords" content="property for rent in dubai, dubai real estate for rent, apartments for rent in dubai, villas for rent in dubai, townhouses for rent in dubai, penthouses for rent in dubai, luxury property for rent dubai, dubai homes for rent, dubai flats for rent, dubai furnished apartments for rent, dubai unfurnished apartments for rent, dubai rental market, dubai real estate agents, dubai property rental market, affordable homes for rent dubai, dubai waterfront homes for rent, zero commission property for rent dubai, dubai gated community homes for rent, family homes for rent in dubai, furnished 1 bhk for rent in dubai, unfurnished apartment for rent dubai, smart home for rent in dubai, penthouse with burj khalifa view for rent, ready to move villa for rent in dubai, houzzhunt, houzzhunt real estate, houzzhunt dubai, houzzhunt rent property, rent dubai property hh, houzzhunt apartments rent, rent villa dubai houzzhunt, houzzhunt property deals, houzzhunt real estate rent, hh premium homes dubai, houzzhunt homes for rent, luxury rent houzzhunt, houzzhunt dubai listings, rent with houzzhunt, dubai homes hh, houzzhunt ready properties, hh rent properties">
...
<!-- parent: .hh-hero-01 -->
<div class="hh-hero-01" style="background-image: url(assets/images/banner/rent-properties-banner.webp);">
    <div class="container">
        <!-- Hero copy -->
        <div class="row">
            <div class="col-12">
                <header>
                    <h1>Premium Rentals in Dubai</h1>
                    <p>Discover handpicked apartments, villas, and townhouses across Dubai's most sought-after
                        communities. Flexible leases, verified listings, and move-in-ready homes tailored for you.</p>
                </header>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <form id="rent-filter-form" method="get" action="rent-properties.php">
                            ...
                            <div class="col-lg-1">
                                <div class="mt-4">
                                    <button
                                        ...
                                        onclick="window.location.href='rent-properties.php';"
                                    >
                                        Reset
                                    </button>
                                </div>
                ...
                <div class="hh-properties-01-head">
                    ...
                    <div>
                        <label>
                            ...
                            <select
                                name="sort"
                                form="rent-filter-form"
                                onchange="document.getElementById('rent-filter-form').submit();"
                            >
                                <option value="price_desc" <?= $filters['sort'] === 'price_desc' ? 'selected' : '' ?>>Price: High to Low</option>
                                <option value="price_asc" <?= $filters['sort'] === 'price_asc' ? 'selected' : '' ?>>Price: Low to High</option>
                                <option value="name_asc" <?= $filters['sort'] === 'name_asc' ? 'selected' : '' ?>>A to Z</option>
                                <option value="newest" <?= $filters['sort'] === 'newest' || $filters['sort'] === '' ? 'selected' : '' ?>>Newest</option>
                                <option value="oldest" <?= $filters['sort'] === 'oldest' ? 'selected' : '' ?>>Oldest</option>
                                <option value="completion_date" <?= $filters['sort'] === 'completion_date' ? 'selected' : '' ?>>Completion Date</option>
                            </select>
                        </label>
        ...
        <div class="row hh-properties-01-grid">
            <?php if ($rentProperties): ?>
                <?php foreach ($rentProperties as $property): ?>
                    ...
                    $primaryImage = $heroBanner !== '' ? $heroBanner : ($galleryImages[0] ?? 'assets/images/banner/rent-properties-banner.webp');
                    ...
                    <div class="col-12 col-md-6 col-lg-4">
                        <a href="rent-properties-details.php?id=<?= (int)($property['id'] ?? 0) ?>" class="property-link">
                            <article>
            ...
            <?php else: ?>
                <div class="col-12">
                    <p class="mb-0">No rental properties available right now. Please check back soon.</p>
                </div>
            <?php endif; ?>
        </div>
